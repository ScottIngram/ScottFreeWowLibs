...
function Throttler:asFunc()
    return function(zelf, ...)
        print("Throttler:asFunc():exe() !!!!!! .....", isTable(zelf) and zelf.ufoType)
        --zebug.error:dumpy("UFO ?",x)
        print(...)
        self:exe(...)
    end
end
    ...
    if runNow then
        ...
        if self.isQueued then
            ...
            C_Timer.After(runWhen, function() -- using ... here is fail.  No args are passed from After into the func
                ...
                zebug.info:--[[ifMe1st(self.id):]]name("exe"):out(7,div, "t", tFormat4(time()), "delayed EXE", self.id, "prolapsed", nFormat4(prolapsed), "xId", exeId)
